...
class Field_of_Study(models.Model):
    ...
    name = models.CharField(
        max_length=100, default="Nieznane studia"
    )  # Nazwa kierunku studiów
    degree = models.CharField(
        max_length=20, choices=DEGREES
    )  # Stopień studiów określony w zmiennej DEGREES
    study_mode = models.CharField(
        max_length=30, choices=STUDY_MODE
    )  # Tryb studiów określony w zmiennej STUDY_MODE
    language = models.CharField(max_length=20, default="Polski")  # Język studiów
    university = models.ForeignKey(
        "University", on_delete=models.CASCADE, to_field="id"
    )  # Klucz obcy do tabeli University, określa uczelnię na której odbywają się studia
    description = models.TextField(max_length=1000)  # Opis kierunku studiów
    link_to_site = models.CharField(
        max_length=255, default="Brak linku do strony"
    )  # Link do strony kierunku studiów
...
class Exam_Subjects(models.Model):
    field_of_study = models.ForeignKey(
        "Field_of_Study", on_delete=models.CASCADE, to_field="id"
    )
...
class Alternative_Exam_Subjects(models.Model):
    main_subject = models.ForeignKey(
        "Exam_Subjects", on_delete=models.CASCADE, to_field="id"
    )
    subject = models.ForeignKey("Subjects", on_delete=models.CASCADE, to_field="id")
